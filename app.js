/* Ironwood 3D Product Demo • v10 • 2026-01-06 */
async function loadModels(){
  const res = await fetch("./models/models.json?v=v10", { cache: "no-store" });
...
async function renderDiagnostics(){
  ...
  try{
    const res = await fetch(`./manifest.json?v=v10`, { cache: "no-store" });
    ...
    for(const [path, meta] of entries){
      try{
        const r = await fetch(`./${path}?v=v10`, { method: "GET", cache: "no-store" });
        const good = r.ok;
        if(good) ok++;
